...
import { CreateWatchlistBody } from '@/lib/validation/watchlist';
...
export async function POST(req: Request, ctx: Ctx): Promise<Response> {
  ...
  const parsed = CreateWatchlistBody.safeParse(json);
  if (!parsed.success) {
    return Response.json({ error: 'invalid_body', issues: parsed.error.issues }, { status: 400 });
  }

  try {
    const entry = addWatchlistEntry(db, portfolioId, parsed.data);
